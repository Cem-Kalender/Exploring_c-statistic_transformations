...
## STEP BY STEP ----
n.imp = 5
data = simulate.multivariate(noise=0.05, n = 10000)
...
# Apply indicator method to verify
# Missingness on V1
# data$missingV1 = ifelse(is.na(trainset$V1), 1, 0)
# lm(missingV1~., data = data) %>% summary()
# data = data%>% select(Y, V2, V1)
# # Missingness on Y
# testdata$missingY = ifelse(is.na(testset$Y), 1, 0)
# lm(missingY~., data = testdata) %>% summary()
# data = testdata%>% select(Y, V2, V1)

# IMPUTE ---
train.imp = mice(trainset, method = c('pmm','pmm','logreg'), n.imp)
test.imp =  mice(testset, method = c('pmm','pmm','logreg'), n.imp)

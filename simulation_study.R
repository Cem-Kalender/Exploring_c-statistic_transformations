...
t(sapply(c(2,1,3,4), function(i){
  sapply(simulation_study, function(x){
    sapply(x, function(z){
      z[i,1]
    })
  }) %>% colMeans()
...
# how broad is the confidence interval actually?
...
lapply(1:3, function(g){
  sapply(c(2,1,3), function(h){
    c_statistic = sapply(1:1000, function(i){simulation_study[[g]][[i]][,1][h]})
    c('skewness'=moments::skewness(c_statistic), 'kurtosis'=moments::kurtosis(c_statistic), 'variance'=sd(c_statistic))
  })
...
# Bias + Monte Carlo SE + CI
ci.bias = lapply(1:3, function(j){
  t(sapply(1:3, function(i){
    lb = biass[i,j] - 1.96*(mcerrors[i,j]/sqrt(1000))
    b = biass[i,j]
    ub = biass[i,j] + 1.96*(mcerrors[i,j]/sqrt(1000))
    cbind('lb'=lb, 'bias'=b, 'ub'=ub)
  }))
})
name = c('theta = 0.7', 'theta = 0.8', 'theta = 0.9')
names(ci.bias) = name

# Difference per sub-study for the pooling methods
par(mfrow=c(1,1))
